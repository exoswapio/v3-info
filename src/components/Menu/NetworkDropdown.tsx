...
import { MesosNetworkInfo, SUPPORTED_NETWORK_VERSIONS } from 'constants/networks'
    ...
    <Container ref={node}>
      <Wrapper onClick={() => setShowMenu(!showMenu)}>
        <RowFixed>
          ...
          {[MesosNetworkInfo].includes(activeNetwork) ? null : (
        ...
        <FlyOut>
          <AutoColumn gap="16px">
            ...
            {SUPPORTED_NETWORK_VERSIONS.map((n) => {
              return (
                <StyledInternalLink key={n.id} to={`${n === MesosNetworkInfo ? '' : '/' + n.route}/`}>
                  <NetworkRow
                    onClick={() => {
                      setShowMenu(false)
                    }}
